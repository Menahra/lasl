...
- [ ] **AC-6**: The page displays a "Display Name" input field, pre-populated with the user's current display name, which the user can edit and save.
...
- [ ] **AC-12**: The "Display Name" field must enforce a minimum length of 2 characters and a maximum length of 50 characters.
- [ ] **AC-13**: Confirming the account deletion with the correct password permanently deletes the user's data from the system, invalidates their session, and redirects them to the logged-out state (e.g., login or home page).
...
| Updating display name to a string shorter than 2 or longer than 50 characters | Shows a validation error indicating the length requirement |
