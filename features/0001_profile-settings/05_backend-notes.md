...
- **Validation**: Added lengths constraints (min 2, max 50) for `firstName` and `lastName` on the `PATCH /api/v1/users/me` endpoint.
...
- The `PATCH /api/v1/users/me` endpoint now enforces `min(2).max(50)` lengths for `firstName` and `lastName`.
